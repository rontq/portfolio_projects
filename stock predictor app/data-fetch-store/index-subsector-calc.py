...
def process_subsector(subsector):
    ...
    for i, date in enumerate(sorted_dates):
        ...
        cur.execute("""SELECT SUM(0.3 * market_cap + 0.7 * market_cap_proxy) FROM stock_market_table WHERE subsector = %s AND date = %s""", (subsector, date))
        current_subsector_cap = cur.fetchone()[0] or 0

        cur.execute("""SELECT SUM(0.3 * market_cap + 0.7 * market_cap_proxy) FROM stock_market_table WHERE sector = %s AND date = %s""", (sector_name, date))
